...
import { X, Clock, Video, Check, ShieldCheck, Sparkles, ArrowRight, Calendar as CalendarIcon, Globe, Download, Copy } from 'lucide-react';
...
END:VCALENDAR`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-150">
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl bg-white border-2 border-stone-200 p-5 sm:p-8 shadow-2xl space-y-5 text-stone-900 animate-in zoom-in-95 duration-150">
          ...
          <form onSubmit={handleNextToDetails} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-700 flex items-center gap-1">
                <Globe className="w-3.5 h-3.5 text-[#66c310]" />
                <span>{language === 'es' ? 'Tu Zona Horaria:' : 'Your Timezone:'}</span>
              </label>
              <select
                value={selectedTimezone}
                onChange={(e) => setSelectedTimezone(e.target.value)}
                className="w-full p-2.5 rounded-xl bg-stone-50 border border-stone-200 text-xs font-bold text-stone-800 focus:bg-white focus:border-[#0d382c]"
              >
                {timezones.map((tz) => (
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-stone-700 flex items-center gap-1">
                <CalendarIcon className="w-3.5 h-3.5 text-[#66c310]" />
                  ...
                  <button
                    ...
                    className={`p-3 rounded-2xl flex flex-col items-center shrink-0 w-16 border transition-all cursor-pointer ${
                      selectedDayIndex === idx
                        ? 'bg-[#0d382c] text-white border-[#0d382c] shadow-md scale-105'
                    ...
                    <button
                      ...
                      className={`py-2 px-2.5 rounded-xl text-xs font-bold border transition-all cursor-pointer text-center ${
                        selectedTimeSlot === slot
                          ? 'bg-[#66c310] text-[#0b2d22] border-[#66c310] shadow-xs'
            ...
            <div className="p-3 rounded-2xl bg-[#f4fbf0] border border-[#d0f4bd] flex items-center justify-between text-xs text-[#0d382c] font-bold">
            ...
            <button
              type="submit"
              className="w-full py-4 rounded-full bg-[#0d382c] hover:bg-[#164c3c] text-white font-black text-sm shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.02]"
            >
          ...
          <form onSubmit={handleFinalBook} className="space-y-4">
            ...
            <div className="space-y-3 pt-1">
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 mb-1.5">
                  {language === 'es' ? 'Selecciona tu Programa:' : 'Select Your Focus Program:'}
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  {[
                    { id: 'basics', title: language === 'es' ? 'Inglés desde 0' : 'English from 0', sub: language === 'es' ? 'Bases' : 'Basics' },
                    { id: 'specific', title: language === 'es' ? 'Inglés Específico' : 'Specific English', sub: language === 'es' ? 'Trabajo/Restaurantes' : 'Work/Life' },
                    { id: 'conversation', title: language === 'es' ? 'Conversación' : 'Conversation Practice', sub: language === 'es' ? 'Confianza' : 'Confidence' },
                  ].map((prog) => (
                    <button
                      key={prog.id}
                      type="button"
                      onClick={() => setSelectedService(prog.id)}
                      className={`p-2.5 rounded-xl border text-left transition-all cursor-pointer ${
                        selectedService === prog.id
                          ? 'bg-[#0d382c] text-white border-[#0d382c] shadow-xs'
                          : 'bg-stone-50 text-stone-700 border-stone-200 hover:bg-stone-100'
                      }`}
                    >
                      <div className="text-xs font-extrabold leading-tight">{prog.title}</div>
                      <div className={`text-[10px] mt-0.5 ${selectedService === prog.id ? 'text-[#66c310]' : 'text-stone-500'}`}>{prog.sub}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 mb-1">
                  {language === 'es' ? '¿En qué te gustaría enfocarte más?' : 'Speaking Goal / Notes for Nick'}
                </label>
                <textarea
                  rows={2}
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder={language === 'es' ? 'Ej. Quiero hablar con seguridad en restaurantes, trabajo o entrevistas.' : 'e.g. Stop translating in my head during meetings and restaurant visits.'}
                  className="w-full px-3.5 py-2.5 rounded-2xl bg-stone-50 border border-stone-200 text-stone-900 text-xs sm:text-sm focus:outline-hidden focus:border-[#0d382c] focus:bg-white transition-colors resize-none"
                />
              </div>
            </div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                type="submit"
                disabled={loading}
                className="flex-1 py-3.5 rounded-full bg-[#0d382c] hover:bg-[#164c3c] text-white font-black text-sm shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
          ...
          <div className="text-center space-y-4 py-2 animate-in zoom-in-95 duration-150">
            ...
            <div className="p-3.5 rounded-2xl bg-stone-50 border border-stone-200 flex items-center justify-between gap-2 text-left">
              <div className="truncate">
                ...
                <span className="text-xs font-mono font-bold text-[#0d382c] truncate">{zoomLink}</span>
              </div>
              <button
                type="button"
                onClick={handleCopyZoom}
                className="px-3 py-1.5 rounded-xl bg-[#0d382c] text-white text-xs font-bold flex items-center gap-1 shrink-0 cursor-pointer"
              >
                <Copy className="w-3 h-3 text-[#66c310]" />
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-3 rounded-2xl bg-[#eefae8] hover:bg-[#dcf5cc] text-[#0d382c] font-black text-xs border border-[#c4eeb0] flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
              >
                <Download className="w-3.5 h-3.5 text-[#66c310]" />
